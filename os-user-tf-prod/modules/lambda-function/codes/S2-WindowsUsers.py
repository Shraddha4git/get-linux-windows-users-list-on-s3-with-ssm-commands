...
CONST_CSV_SUFFIX = os.getenv("CSV_SUFFIX", "-windows-users-list")
CONST_SKIP_TAG_KEY = "OSUserScriptExclude"

USER = "User(OS)"
ADMIN_USER = "Admin User(OS)"
UPDATED_AT = "Updated At (User Data)"
'''
UAT_SERVER1 = {
    "InstanceId": "i-024549cc00c9918c6",
    USER: None,
    ADMIN_USER: None,
    UPDATED_AT: "2025-04-29"
}

FIXED_INSTANCES = [UAT_SERVER1]
'''
FIXED_INSTANCES = []

CONST_USR_CMD = "Get-LocalUser | Where-Object { $_.Enabled -eq $true } | Select-Object -ExpandProperty Name"
CONST_ADMIN_USR_CMD = "Get-LocalGroupMember -Group 'Administrators' | ForEach-Object { $_.Name.Split('\\')[-1] }"

def put_users_to_server_list(server_list, users, admin_users):
    logger.info("Mapping users and admin users to server list...")
    for user in users:
    ...
    for admin_user in admin_users:
        for server in server_list:
            if admin_user[0] == server["InstanceId"]:
                server[ADMIN_USER] = admin_user[1]
                server[UPDATED_AT] = datetime.datetime.now().strftime("%Y-%m-%d")
                break
    return server_list
...
def get_command_output(ssm, cmd_id, instance_id):
    try:
        ...
        logger.info(f"Failed to get command output for instance {instance_id}: {e}")
        return None
    ...
    for instance_id in run_command_ids:
        try:
            ...
            if output:
                ...
                error = output.get('StandardErrorContent', '').strip()

                logger.info(f"{region} - Standard Output for {instance_id}: {result}")
                logger.info(f"{region} - Standard Error for {instance_id}: {error}")

                if result:
                    results.append([instance_id, result])
                    executed_instances.append(instance_id)
                elif error:
                    logger.warning(f"{region} - PowerShell error for {instance_id}: {error}")
                    command_status = "Fail"
                    failed_to_send_command_ids.append(instance_id)
                else:
                    logger.warning(f"{region} - No output returned for {instance_id}")
                    command_status = "Fail"
                    failed_to_send_command_ids.append(instance_id)
            else:
                command_status = "Fail"
            ...
            logger.warning(f"{region} - Failed to execute SSM command on {instance_id}: {e}")
            command_status = "Fail"
            failed_to_send_command_ids.append(instance_id)

    logger.info(f"{region} - failed_to_send_command_ids: {failed_to_send_command_ids}")
...
def describe_instances(region, ec2, environment, specific_instances_windows):
    logger.info(f"Describing instances in {region}...")
    servers = []
    run_command_ids = []

    try:
        if specific_instances_windows:
            logger.info(f"{region} - Fetching specified Windows instances: {specific_instances_windows}")
            resp = ec2.describe_instances(InstanceIds=specific_instances_windows)
        else:
        ...
        for res in resp['Reservations']:
            for inst in res['Instances']:
                if inst.get('Platform', '').lower() != 'windows':
                    logger.info(f"{region} - Skipping non-Windows instance: {inst['InstanceId']}")
                    continue
                ...
                if any(inst["InstanceId"] == f["InstanceId"] for f in FIXED_INSTANCES):
                    for f in FIXED_INSTANCES:
                        if inst["InstanceId"] == f["InstanceId"]:
                            inst_data.update({USER: f[USER], ADMIN_USER: f[ADMIN_USER], UPDATED_AT: f[UPDATED_AT]})
                            break
        ...
        logger.info(f"{region} - Failed to describe instances: {e}")

    return servers, run_command_ids

def process_region(region, output_list, executed_set, command_status_set, environment, region_failed_ids, specific_instances_windows):
    logger.info("process_region() START")
    ec2 = boto3.client('ec2', region_name=region)
    servers, run_command_ids = describe_instances(region, ec2, environment, specific_instances_windows)
    ...
    if run_command_ids:
        ...
        admin_users, executed_admin_users, _, _ = run_command(instance_ids_success, region, CONST_ADMIN_USR_CMD)        
        executed_set.update(executed_admin_users)

        servers = put_users_to_server_list(servers, users, admin_users)
...
def lambda_handler(event, context): 
    ...
    response = {
        "L2_Windows_IsError": False,  # Default to no error
        "CSVFilePathWindows": None,
        "windows_ssm_fail_ids": [],
        "Command_Windows_Status": "Fail"
    }
    ...
    if not environment:
        logger.info("Environment key is not present in the input")
        response["L2_Windows_IsError"] = True
    ...
    if specific_instances_linux and not specific_instances_windows:
    ...
    if not regions:
        logger.info("Regions key is missing.")
        response["L2_Windows_IsError"] = True
    ...
    for region in regions:
        if region not in REGIONS:
            logger.info(f"Region value ({region}) is not correct. Accepted values are {REGIONS}")
            response["L2_Windows_IsError"] = True
    ...
    for region in regions:
        ...
        t = threading.Thread(target=process_region, args=(region, region_result, region_executed, region_command_status, environment, region_failed_ids, specific_instances_windows))
    ...
    for t, region_result, region_executed, region_command_status, region_failed_ids in threads:
        ...
        windows_ssm_fail_ids.extend(region_failed_ids) 

        # Record executed instances
    executed_windows_instance_ids = list(executed_instance_ids_set)
    logger.info(f"Commands executed on {len(executed_windows_instance_ids)} instance(s).")
    ...
    if results:
        ...
        response["CSVFilePathWindows"] = filename
    else:
        logger.info("No instances processed. CSV will not be created.")

    # remove duplicates from list
    windows_ssm_fail_ids = list(set(windows_ssm_fail_ids))

    # Determine command status
    response["Command_Windows_Status"] = "Fail" if "Fail" in command_status_set else "Success"
    response["windows_ssm_fail_ids"] = windows_ssm_fail_ids
